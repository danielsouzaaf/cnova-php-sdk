...
class ApiClient {
  ...
  /*
   * @var string user agent of the HTTP request, set to "PHP-Client" by default
   */
  protected $user_agent = "PHP-Client";
  ...
  function __construct($host = null) {
    if ($host === null) {
      $this->host = 'https://desenvolvedores.cnova.com/api-portal/proxy/';
    } else {
      $this->host = $host;
    }
  }
  ...
  public function updateParamsForAuth(&$headerParams, &$queryParams, $authSettings)
  {
    ...
    foreach($authSettings as $auth) {
      ...
      switch($auth) {
        ...
        case 'client_id':
          $headerParams['client_id'] = $this->getApiKeyWithPrefix('client_id');
          
          break;
        
        case 'access_token':
          $headerParams['access_token'] = $this->getApiKeyWithPrefix('access_token');
